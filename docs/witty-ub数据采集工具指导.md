...
| --umq-log-path | string | 指定umq日志路径 | 1.pod-mode=on时，该参数格式需要为pod_id:path列表，即多个pod时格式为 pod_id1:path1,pod_id2:path...，其中pod_id需要符合k8s pod的命名规范（只包含小写字母，数字和连字符-，并且不能以连字符-开头或者结尾），pod_id不能重复，path为文件或者目录的绝对路径，不允许包含逗号“,”<br> 2.pod=off时，该参数为某个文件或者目录的绝对路径，不支持多个路径的形式即不支持"path1,path2"，如不指定，默认为```/var/log/messages```<br>3.当日志路径为目录时，会读取该目录下所有后缀为.log文件的内容，如该路径没有后缀为.log文件则跳过 | pod=on时必选 |
...
|:--------:|:----------:|:---------|:---------|:-------:|
| --pod-mode | string | 指定是否是容器化部分 | 需为on(容器化部署)或者off(非容器化部署) | 是 |
| --ubsocke-log-path | string | 指定UBsocket的日志路径 | 1.pod=on时，该参数格式需要为pod_id:path列表，即多个pod时格式为 pod_id1:path1,pod_id2:path<br> 2.pod=off时，该参数为某一日志文件路径，不支持多个文件的形式，如不指定，默认为```/var/log/messages``` | pod=on时必选 |
| --umq-log-path | string | 指定UMQ日志路径 | 1.pod=on时，该参数格式需要为pod_id:path列表，即多个pod时格式为 pod_id1:path1,pod_id2:path<br> 2.pod=off时，该参数为某个目录路径，不支持多个目录的形式，如不指定，默认为```/var/log/umdk/umq/```目录下所有文件 | pod=on时必选 |
| --liburma-log-path | string | 指定libURMA日志路径 | 1.pod=on时，该参数格式需要为pod_id:path列表，即多个pod时格式为 pod_id1:path1,pod_id2:path<br> 2.pod=off时，该参数为某个目录路径，不支持多个目录的形式，如不指定，默认为```/var/log/umdk/urma/```目录下所有文件 | pod=on时必选 |
| --urmacore-log-path | string | 指定URMA内核日志路径 | 如不指定默认为dmesg | 否 |
| --libudma-log-path | string | 指定libUDMA日志路径 |  1.pod=on时，该参数格式需要为pod_id:path列表，即多个pod时格式为 pod_id1:path1,pod_id2:path<br> 2.pod=off时，该参数为某一日志文件路径，不支持多个文件的形式，如不指定，默认为```/var/log/messages``` | pod=on时必选 |
| --udmacore-log-path | string | 指定UDMA内核日志路径 | 如不指定默认为dmesg | 否 |
...
* 容器化部署
    ...
    --strat-time 2026-03-09 00:00:00 --end-time 2026-03-10 00:00:00
    ```
* 非容器化部署
    ```shell
    witty-ub-log --pod_mode on \
